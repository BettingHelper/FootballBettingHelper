...
public class WindowWithProgressBar extends JFrame{
    ...
    public WindowWithProgressBar(String text) {
        ...
        try{
            String resultOfDownload = FTPLoader.downloadFile(Settings.getLogin(n), Settings.getPassword(n), "/data/football/database/updates/" + fileName, Settings.TMP_DIR + fileName);
                ...
                try {
                    ...
                    while (((str = in.readLine()) != null)) {
                        if (Integer.parseInt(str.split("_")[0]) >= Integer.parseInt(lastUpdateName.split("_")[0])){
                            resultOfDownload = FTPLoader.downloadFile(Settings.getLogin(n), Settings.getPassword(n), "/data/football/database/updates/" + str, Settings.TMP_DIR + str);
