...
public class PanelStart extends JPanel{
    ...
    public PanelStart() {
        ...
        JLabel label1 = new JLabel("Добро пожаловать в Football Betting Helper!");
        ...
        String text = "<html><br><blockquote>Программа 'Football Betting Helper' - это:</blockquote>";
        ...
        String tezis = "<html>" +
                "<blockquote>- актуальная статистика топовых футбольных лиг;</blockquote>" +
                "<blockquote>- простота и удобство интерфейса;</blockquote>" +
                "<blockquote>- разнообразные таблицы и графики;</blockquote>" +
                "<blockquote>- разбивка статистики по таймам;</blockquote>" +
                "<blockquote>- тренды команд и лиг;</blockquote>" +
                "<blockquote>- статистика арбитров топовых чемпионатов;</blockquote>" +
                "<blockquote>- автоматическое обновление данных;</blockquote>" +
                "<blockquote>- экономия времени при анализе матчей и т.д.</blockquote></html>";
        ...
        try {
            ...
            String resultOfDownload = FTPLoader.downloadFile(Settings.getLogin(n), Settings.getPassword(n),
                    "/data/football/notifications/notifications.txt", Settings.TMP_DIR + "notifications.txt");
        ...
        String legal = "<html> (<a href=\"legalbet.ru/blogs/football-betting-helper\">legalbet.ru/blogs/football-betting-helper</a>) </html>";
        ...
        legalLabel.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                URI uri = null;
                try {
                    uri = new URI("https://legalbet.ru/blogs/football-betting-helper");
                } catch (URISyntaxException e1) {
                    e1.printStackTrace();
                }
                try {
        ...
        String newestVersionFile = Settings.TMP_DIR + "newestVersionFBH.txt";
        int n = Settings.getNumberOfAccount();
        try {
            String resultOfDownload = FTPLoader.downloadFile(Settings.getLogin(n), Settings.getPassword(n),
                    "/versions/newestVersionFBH.txt", newestVersionFile);
            try {
        ...
        String version = "Текущая версия - FBH_" + currentVersion;
        if (!currentVersion.equals(newestVersion)){
            version += ". ДОСТУПНА ВЕРСИЯ - FBH_" + newestVersion + ". Для ее получения перезапустите программу.";
            /*try {
